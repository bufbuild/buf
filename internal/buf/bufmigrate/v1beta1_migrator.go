...
# remove this comment when you're finished.
`
	bufWorkHeaderWithName = `# Generated by %q. Edit as necessary, and
# remove this comment when you're finished.
#
# This workspace file replaces the %q found in
# the %q module.
`
	bufWorkHeaderWithoutName = `# Generated by %q. Edit as necessary, and
# remove this comment when you're finished.
#
# This workspace file replaces the %q found in
# the previous configuration.
...
func (m *v1beta1Migrator) maybeMigrateConfig(dirPath string) (bool, error) {
	...
	if len(buildConfig.RootToExcludes) > 1 {
		// If we have more than one root, we need to create
		// a workspace file.
		...
		header := fmt.Sprintf(bufWorkHeaderWithName, m.commandName, bufconfig.ExternalConfigV1Beta1FilePath, v1beta1Config.Name)
		if v1beta1Config.Name == "" {
