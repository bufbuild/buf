...
import { pkgAndSubpathForCurrentPlatform, downloadedBinPath } from "./platform";
...
function validateBinaryVersion(...command: string[]): void {
  ...
  const stdout = child_process
    ...
    .toString()
    .trim();
  // strip any trailing revision from current version when comparing. We only care about the major, minor, and patch versions.
  const currentVersionWithoutRevision = CURRENT_VERSION.split("-")[0];
