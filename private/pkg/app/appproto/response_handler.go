...
func (h *responseHandler) HandleResponse(
	...
	handleResponseOptions := newHandleResponseOptions()
	for _, option := range options {
		option(handleResponseOptions)
	}
	for _, file := range response.File {
		if file.GetInsertionPoint() != "" {
			if handleResponseOptions.insertionPointReadBucket == nil {
				return storage.NewErrNotExist(file.GetName())
			}
			if err := applyInsertionPoint(ctx, file, handleResponseOptions.insertionPointReadBucket, writeBucket); err != nil {
				return err
			}
...
type handleResponseOptions struct {
	insertionPointReadBucket storage.ReadBucket
}

func newHandleResponseOptions() *handleResponseOptions {
	return &handleResponseOptions{}
}
