...
func scaffoldGitRepository(t *testing.T, runner command.Runner) string {
	...
	runInDir(t, runner, local, "git", "checkout", "-b", "buftest/branch1")
	writeFiles(t, local, map[string]string{
	...
	runInDir(t, runner, local, "git", "push", "--follow-tags", "origin", "buftest/branch1")

	// (3) branch off branch and begin work
	runInDir(t, runner, local, "git", "checkout", "-b", "buftest/branch2")
	writeFiles(t, local, map[string]string{
	...
	runInDir(t, runner, local, "git", "push", "--follow-tags", "origin", "buftest/branch2")

	// (4) merge first branch
	runInDir(t, runner, local, "git", "checkout", DefaultBranch)
	runInDir(t, runner, local, "git", "merge", "--squash", "buftest/branch1")
	...
	runInDir(t, runner, local, "git", "checkout", DefaultBranch)
	runInDir(t, runner, local, "git", "merge", "--squash", "buftest/branch2")
	runInDir(t, runner, local, "git", "commit", "-m", "third commit")
	runInDir(t, runner, local, "git", "tag", "v3.0")
	runInDir(t, runner, local, "git", "push", "--follow-tags")

	// commit a local-only branch
	runInDir(t, runner, local, "git", "checkout", "-b", "buftest/local-only")
