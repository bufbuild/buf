...
const (
	...
	defaultBufPolicyYAMLFileVersion = FileVersionV1
)
...
func ReadBufPolicyYAMLFile(reader io.Reader) (BufPolicyYAMLFile, error) {
	return readFile(reader, "", readBufPolicyYAMLFile)
}
...
// externalBufPolicyYAMLFileV1 represents the v1 buf.policy.yaml file.
type externalBufPolicyYAMLFileV1 struct {
...
func readBufPolicyYAMLFile(
	...
	fileVersion, err := getFileVersionForData(data, allowJSON, true, bufPolicyYAMLFileNameToSupportedFileVersions, FileVersionV1, defaultBufPolicyYAMLFileVersion)
	if err != nil {
		fmt.Println("failing here?")
		return nil, err
	}
	fmt.Println("got it?", fileVersion)
	if fileVersion != FileVersionV1 {
		return nil, newUnsupportedFileVersionError(objectData.Name(), fileVersion)
	}
	var externalBufPolicyYAMLFile externalBufPolicyYAMLFileV1
	if err := getUnmarshalStrict(allowJSON)(data, &externalBufPolicyYAMLFile); err != nil {
		fmt.Println("WAT?", err, "->", string(data))
		return nil, fmt.Errorf("invalid as version %v: %w", fileVersion, err)
	}
	fmt.Println("GOT", externalBufPolicyYAMLFile)

	// Policy V1 uses V2 buf.yaml configs.
	bufYAMLFileVersion := FileVersionV2
	...
	for _, externalPluginConfig := range externalBufPolicyYAMLFile.Plugins {
		fmt.Println("Trying to get plugin", externalPluginConfig)
...
func writeBufPolicyYAMLFile(writer io.Writer, bufPolicyYAMLFile BufPolicyYAMLFile) error {
	...
	externalBufPolicyYAMLFile := externalBufPolicyYAMLFileV1{
