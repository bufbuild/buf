...
func parsePluginFullNameComponents(path string) (registry string, owner string, name string, err error) {
	slashSplit := strings.Split(path, "/")
	if len(slashSplit) != 3 {
		return "", "", "", newInvalidPluginFullNameStringError(path)
	}
...
func parsePluginRefComponents(path string) (registry string, owner string, name string, ref string, err error) {
	// split by the first "/" to separate the registry and remaining part
	slashSplit := strings.SplitN(path, "/", 2)
	if len(slashSplit) != 2 {
		return "", "", "", "", newInvalidPluginRefStringError(path)
	}
	...
	registry, owner, name, err = parsePluginFullNameComponents(registry + "/" + colonSplit[0])
	if err != nil {
		return "", "", "", "", newInvalidPluginRefStringError(path)
	}
...
func newInvalidPluginFullNameStringError(s string) error {
	return &ParseError{
...
func newInvalidPluginRefStringError(s string) error {
	return &ParseError{
		typeString: "plugin reference",
		input:      s,
