...
func testResumeProtectedBranchOverlapWithSyncedBranches(t *testing.T, handler TestHandler, run runFunc) {
	...
	var (
		originalBranch = repo.DefaultBranch() // this makes the branch protected
		forkedBranch   = "forkedBranch"
	)

	// (1) commit module and sync protected branch frist
	...
	require.Len(t, plan.ModuleTagsToSync(), 0)

	// (2) commit 5 commits, _don't_ sync;
	// 6 commits would have been be synced if we synced again
	doEmptyCommitAndTag(5)

	// (3) fork branch, commit twice, sync;
	...
	// (4) go back to original branch, commit 4 more times and sync this branch again;
	// branch is protected, so we should start from the last sync point (commit 0): 12 commits should be synced
