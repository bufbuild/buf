...
// archiveBucketCacheKey identifies the contents of an unarchived archive. The
// unarchived bucket is cached rather than the archive itself so that the bytes
// are held once, and so that the archive is expanded once.
type archiveBucketCacheKey struct {
	path            string
	fileScheme      FileScheme
...
func newArchiveBucketCacheKey(archiveRef ArchiveRef) archiveBucketCacheKey {
	return archiveBucketCacheKey{
		path:            archiveRef.Path(),
		fileScheme:      archiveRef.FileScheme(),
		archiveType:     archiveRef.ArchiveType(),
		compressionType: archiveRef.CompressionType(),
		stripComponents: archiveRef.StripComponents(),
	}
}
