...
func TestFormatterWithDeprecation(t *testing.T) {
	...
	testDeprecateNoDiff(t, "enum_value", "testdata/deprecate", []string{"test.deprecate", "test.deprecate.Status.STATUS_ACTIVE", "test.deprecate.Status.STATUS_INACTIVE"},
...
func testDeprecateNoDiff(t *testing.T, name string, path string, deprecatePrefixes []string, files []string) {
	t.Run(name, func(t *testing.T) {
		...
		// Filter bucket to only include specified files
		...
		// Helper to compare formatted bucket to golden files
		assertGolden := func(formatBucket storage.ReadBucket) {
			err := storage.WalkReadObjects(
				ctx,
				formatBucket,
				...
				func(formattedFile storage.ReadObject) error {
					...
					if !strings.HasSuffix(formattedFile.Path(), ".proto") {
						return nil
					}
					formattedData, err := io.ReadAll(formattedFile)
					require.NoError(t, err)
