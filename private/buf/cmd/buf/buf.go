...
func wrapError(err error) error {
	...
	if isConnectError {
		...
		case connectCode == connect.CodeUnauthenticated, isEmptyUnknownError(err):
			if authErr, ok := bufconnect.AsAuthError(err); ok {
				if authErr.HasToken() && authErr.TokenEnvKey() != "" {
					return fmt.Errorf("Failure: the %[1]s environment variable is set, but is not valid. "+
						...
						"visit https://buf.build/docs/bsr/authentication",
						authErr.TokenEnvKey(),
					)
				}
				if authErr.HasToken() {
				...
				return fmt.Errorf("Failure: you are not authenticated for %s. Create a new entry in your netrc, "+
