...
> This breaking change policy does _not_ apply to commands behind the `buf beta` gate. You should expect breaking changes to commands like `buf beta registry`. Our breaking change policy does go into effect, however, when those commands are elevated out of beta.
